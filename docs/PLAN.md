...
- [x] 2026-09-05 **Unassigned-input messaging**: the hook reports every F13–F24 event (any modifier namespace) and swallows only reserved keys; the engine logs and sends an `unassigned` IPC message for any key+modifier combination no input uses; the UI shows a banner ("Received Shift+F16 … no input uses it") with a button that prefills the Inputs add row. `tools/keymon.ps1` prints raw key events with the modifier state the hook reads and the delivery lag. Verified on hardware that a modifier namespace arrives as modifier-down, then F-key-down with the modifier already held.
- [x] 2026-09-05 **Unbound-gesture messaging**: a decoded gesture with no binding in the active profile or Default now logs at info and sends an `unbound` IPC message; the detect strip shows "Detected Tune / Swipe down · 3 fingers in Default, but nothing is bound for it" with a Bind button that opens the picker. Found because the user's 2- and 3-finger inputs were configured correctly but Default only binds the 1-finger gestures.
- [ ] 2026-09-05 **Hold shortcuts: ROLLED BACK the engine side** (modifiers stuck down, every later gesture became Alt+gesture). Kept: `hold_ms` on `Action::Keys` and the "keep the modifiers held for N seconds" checkbox in the record tab (stored, inert), and catalog-based naming of recorded chords. `Action::Release` remains in the model as a no-op. The user will specify the intended behaviour. Previous design for reference: `Action::Keys` gains optional `hold_ms`; the executor keeps the chord's modifiers down afterwards, later chords run on top of them, a holding chord with other modifiers swaps them, and they are let go by `Action::Release` or after `hold_ms` of quiet (crossbeam timer in the pipeline loop). Picker: "Keep the modifiers held afterwards for N seconds" in the Keyboard shortcut tab, "Release held keys" under Other, and a recorded chord is named from the catalog when it matches a documented action (app, system, or generic).
- [x] 2026-09-05 **System + God Mode**: the fallback profile is named "System" (configs saying "Default" migrate on load). New `[god_mode]` profile: its bindings win over every other profile whatever is in the foreground (resolver checks it first; Handled.profile reports "God Mode"). UI: ⚡ God Mode row pinned above System in Active, a warning note on its page, app/System rows bound in God Mode show as locked "⚡ God Mode: …". Intended home for system-wide gestures (window switching, desktops), especially on macOS.
- [x] 2026-09-05 **Icon**: OpenFlow's Touch glyph (`assets/touch.png`, from openflow/frontend/public/modules/touch.png, recoloured near-black) is the icon everywhere: tray (pre-rendered `assets/tray-32.rgba` via include_bytes), engine exe resource (`assets/icon.ico`), Tauri window/bundle icons, sidebar brand mark. Regenerate with `python tools/make_icons.py`.
- [x] 2026-09-05 **Sortable Input column**: the header on every mapping table and on the Inputs page cycles fingers → module → gesture → A–Z (tie-breakers: fingers→module→gesture, module→gesture→fingers, gesture→module→fingers, alpha→module→fingers); default is fingers, where the dial (no count) pins to the top. Shared setting, remembered in localStorage. Possible follow-up: module / finger-count filter chips on Inputs.
- [x] 2026-09-05 **Modifier namespaces by timing**: the hook tracks physical modifier presses itself (no GetAsyncKeyState); a modifier is part of the module's key only if it went down within `engine.namespace_window_ms` (default 100) before the F-key, otherwise it is the user's, reported as `held`, logged, and ignored for decoding. Decided with the user after Alt held by hand turned every gesture into Alt+F-key. NOT built (test first): burst guard (module holding one modifier across a run of repeats); "held modifier as a layer" per-input flag (F22 vs Alt-held+F22 as two inputs).
- [x] 2026-09-05 **Streamed gestures: collapse / follow.** `SemanticEvent::streams()` = 2-finger swipes (Tune measurement; Touch TBD). Input option `follow` (Inputs page checkbox, 2-finger swipes only) is the input-wide default; each binding can override with a tri-state select (Input default / Collapse to one / Follow the swipe) in place of Acceleration. Collapse = first key acts, keys within `engine.stream_gap_ms` (300) are the same swipe and dropped; Follow = every key acts, acceleration forced to 1. Config: `[transport.X] follow = true`, `[..bindings.X] follow = true|false`. Test: `streamed_gesture_collapses_unless_told_to_follow`.
- [x] Background (2026-09-05): Tune 2-finger swipes stream 11–20 key presses per gesture (`capture.log`, 2026-09-05); 1- and 3-finger swipes, taps and detents send one. Modifier namespaces on those bursts are per-event (Shift pressed/released around every key, ≤1 ms), so the timing rule needs no burst guard on the Tune. User suspects the stock profile's 2-finger scroll axes / pinch&spread make the firmware poll those fields; checking whether it is a flash setting before deciding between engine-side burst collapsing and axis-only actions for those fields.
- [x] 2026-09-06 **Windows installer + release workflow**: Tauri NSIS bundle with the engine as a sidecar (`bundle.externalBin`), per-user install, Start Menu folder, hooks (`ui/src-tauri/nsis/hooks.nsh`) that stop both exes before install/uninstall, start the engine after install and remove the Run-key entry on uninstall; `tools/build_installer.ps1` builds it end to end (output `target/release/bundle/nsis/Create Companion_<ver>_x64-setup.exe` + .sha256); `.github/workflows/release.yml` builds on `v*` tags, checks the tag matches tauri.conf.json's version, uploads to a GitHub Release (signing step reserved). UI: Start engine button/dot. First local build: 2.0 MB installer, not installed on the dev machine.
...
- [x] 2026-09-10 **Touch measured.** Phase 1 on the stock profile (`tools/plans/stock-touch.txt`, logged by the new `tools/input_capture.ps1` + `input_analyze.py`, which hook keyboard AND mouse and log every event per gesture) and Phase 2 flashed with F-keys (`census-touch.txt`, `census-touch.module-profile.json`: 2-finger axes Ctrl+F13–F16, 3-finger swipes Alt+F13–F16 + tap Alt+F22, 4-finger swipes Alt+F18–F21 + tap Alt+F22). Streams: the 2-finger scroll axes in every direction (one key per report, 9–11 for half the pad, none while resting) and 4-finger swipe up/down (scaled to distance: 1 flick, 5–6 half pad, 10 whole pad). One key: 3-finger swipes at any speed, 4-finger left/right, every tap; a double tap is two taps ~240 ms apart. A keypress binding changed no field's behaviour. Modifiers are per-event (≤5 ms), so the timing rule holds. `SemanticEvent::streams()` is now per module (Tune unchanged), mirrored in `ui/src/events.ts`; tests `streams_per_module`, `touch_runs_collapse_like_tune_swipes`. Not flashable yet, mirroring NayaFlow's gating: every 1-finger field and the 2-finger tap (a probe write test comes first); 1-/2-finger swipes, double taps and pinch&spread have no known device field (candidates: unbound slots 0x11/0x12/0x14/0x1a/0x1f–0x21, keypress fields 0x22/0x23). Found: OpenFlow's Touch 2-finger axis labels were swapped the way the Tune's were before 2026-09-04 (0x0D/0x0E fire on vertical, 0x0F/0x10 on horizontal); fixed in the parent repo's `AXIS_HALVES["TOUCH"]` the same day (nayaos 80e70c3), so a Touch flashed before that needs one re-flash for the census plan's 2-finger keys to land as written. The unflashed 1-finger firmware fields leak a click or a few cursor pixels while fingers land or lift on multi-finger gestures; a settle guard in the engine is a possible follow-up.
